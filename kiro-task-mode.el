...
(defun kiro-task-save-chat ()
  ...
  (let* ((base-name (if (buffer-file-name)
         ...
         (task-name (if (string-match-p "^-*$" task-name) "chat" task-name))
         (timestamp (format-time-string "%Y%m%d-%H%M%S"))
         (date-path (format-time-string "%Y/%m/%d"))
         (full-dir (expand-file-name (format "chats/%s" date-path) default-directory))
         (save-file (expand-file-name (format "chats/%s/%s-%s.json" date-path task-name timestamp) default-directory))
         (cmd (format "/chat save %s" save-file)))
    ...
    (if (get-buffer-process (current-buffer))
        (progn
          ...
          (insert cmd)
          (comint-send-input))
      (message "⚠️ Not in active shell"))))

(defun kiro-task-save-compact-save ()
  "Save chat, compact it, and save compacted version"
  (interactive)
  (let* ((base-name (if (buffer-file-name)
                        (file-name-nondirectory (buffer-file-name))
                      (buffer-name)))
         ...
         (full-dir (expand-file-name (format "chats/%s" date-path) default-directory))
         (save-file (expand-file-name (format "chats/%s/%s-%s.json" date-path task-name timestamp) default-directory))
         (compact-file (expand-file-name (format "chats/%s/%s-%s-compacted.json" date-path task-name timestamp) default-directory)))
    (make-directory full-dir t)
    (if (get-buffer-process (current-buffer))
        (progn
          (goto-char (point-max))
          (insert (format "/chat save %s" save-file))
          (comint-send-input)
          (sit-for 0.5)
          (insert "/compact")
          (comint-send-input)
          (sit-for 0.5)
          (insert (format "/chat save %s" compact-file))
          (comint-send-input))
      (message "⚠️ Not in active shell"))))
...
(defvar kiro-task-mode-map
  (let ((map (make-sparse-keymap)))
    ...
    (define-key map (kbd "C-c k c") 'kiro-task-save-compact-save)
